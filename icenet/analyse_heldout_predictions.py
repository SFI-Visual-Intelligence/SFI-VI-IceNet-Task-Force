...
if __name__ == "__main__":
    ...
    if pre_load_results_df:
        ...
        if len(results_df_fnames) >= 1:
            ...
            print(
                "\n\nLoading previous results dataset from {}".format(
                    old_results_df_fpath
                )
            )

        # Load previous results, do not interpret 'NA' as NaN
        results_df = pd.read_csv(
            old_results_df_fpath, keep_default_na=False, comment="#"
        )
        ...
        new_models = [
            model for model in model_compute_list if model not in existing_models
        ]
        new_metrics = [
            metric for metric in metric_compute_list if metric not in existing_metrics
        ]
